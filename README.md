# Дизассемблирование и поиск уязвимостей в исполняемом файле

## Введение

Данный проект является учебным. Предлагался студеннтам 1 курса МФТИ. В начале 2 семестра мы работали в операционной системе DOS.
Мы делали это исключительно в образовательных целях, т.к. в DOS можно свободно работать на очень низком уровне. 
В данной задаче было предложено написать crackme, разбиться на пары и обменяться файлами для взлома с напарником.
...
Первая часть задачи состояла в том, чтобы проанализировать программу с помощью дизассемблера и отладчика, найти уязвимости и, используя их,
заставить программу вывести сообщение о том, что введённый пароль верный.

Вторая часть задачи заключалась в том, чтобы написать программу с графическим интерфейсом, которая бы меняла байты в предоставленной программе,
...
Первым делом был произведён поиск строк в исходном файле crackme.com. Для того, чтобы увидеть какие строки есть в программе, были использованы комментарии, которые вставляет IDA в view-режиме "Disassembly"
после строк с объявлением байтов (db).

![Снимок экрана 2025-03-05 214921](https://github.com/user-attachments/assets/d4ef9923-2b35-4469-8a19-20ca068bb3f2)
...
Итак, адрес "правильного" сообщения — DDh + 100h = 1DDh.
...
1EEh. Уже отсюда можно сделать вывод, что способ определения правильности введённого пароля — хэширование, правильный хэш
лежит по адресу 1EEh. Если BX равен правильному хэшу, вызывается функция sub_6F, выводящая сообщение о правильности введённого
...
Для этого необходимо предварительно вычесть из 1B5h 100h, чтобы учесть PSP. По адресу B5h в файле crackme.com лежит строка
...
буфера для ввода очень большой — FFFFh - 1ABh. При этом адрес буфера для ввода - 1ABh (dx := 1ABh в функции sub_28).
...
Создадим с помощью кода на Python файл размером FFFFh - 1ABh - 6h байт, состоящий, из нулевых байтов.
"0FFFFh - 1ABh" — это размер вводимого буфера. Вычитание 6 байт нужно, потому что мы меняем только значение, находящееся
по адресу FFF8h, то есть байты с FFFAh по FFFFh (всего 6 байт) мы не трогаем.
...
Таким образом программа будет выдавать сообщение о том, что введённый пароль верный, если он не является таковым.
